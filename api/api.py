...
#         if current_similarity > similarity and prev_id != face_id:
#             matching_face_id = face_id
#             similarity = current_similarity
#             prev_id = matching_face_id

#     return matching_face_id, similarity
        
def checking_id(features, data, id_list):
    matching_face_id = None
    similarity = CHECK_THRESHOKD
    for face_id, face_data in data.items():
        existing_features = np.array(face_data['extract_feature'])
        ...
        if current_similarity > similarity:
            check = not any(face_id == id for id in id_list)
            if check:
                matching_face_id = face_id
                similarity = current_similarity
    return matching_face_id, similarity
...
async def process_data(data_str, websocket: WebSocket):
        ...
        for (x,y,width,height) in faces:
            ...
            matching_face_id = finding_id(features, data)
